...
#define BATT_ADC_LEVEL_4_2V           1737
#define BATT_ADC_LEVEL_3_4V           1406   
...
static uint16 battMinLevel = BATT_ADC_LEVEL_3_4V; // For VDD/3 measurements
static uint16 battMaxLevel = BATT_ADC_LEVEL_4_2V; // For VDD/3 measurements
...
uint8 battMeasure( void )
{
 ...
 // HalAdcSetReference( HAL_ADC_REF_125V );
  ...
  else
  {
    ...
    {
      percent[i++] = battServiceCalcCB(adc);
    }
    else
    {
      percent[i++] = (adc - battMinLevel)>>1;
     ...
     // percent = (uint8) ((((adc - battMinLevel) * 100) + (range - 1)) / range);
    }
  }

  if(i < 16)
  {
    battLevel = percent[i-1];
  }
  else
  {
    i = 15;
