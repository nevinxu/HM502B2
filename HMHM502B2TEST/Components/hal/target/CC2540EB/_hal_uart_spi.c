...
static void HalUARTInitSPI(void)
{
  ...
  UxGCR =  0x40 | 0x20 | 15;                       /* Cfg for the max Rx/Tx baud of 2-MHz */
 // UxGCR =  15;                       /* Cfg for the max Rx/Tx baud of 2-MHz */
  ...
  HAL_DMA_SET_WORD_SIZE( ch, HAL_DMA_WORDSIZE_BYTE );    //×Ö½Ú
   ...
   */
  HAL_DMA_SET_WORD_SIZE( ch, HAL_DMA_WORDSIZE_WORD );    //½ÓÊÕ
...
static void HalUARTOpenSPI(halUARTCfg_t *config)
{
  ...
  UxCSR |= CSR_RE;   //´ò¿ª½ÓÊÕ

  PxIFG = 0;
  PxIF = 0;
  IENx |= IEN_BIT;     //½ÓÊÕÖÐ¶Ï´ò¿ª
